...
public class MockFileSystemAssertions
{
    ...
    public AndConstraint<MockFileSystemAssertions> ContainDirectory(string rawPath, string because = "",
        params object[] becauseArgs)
    {
        var path = FixPath(rawPath);
        Execute.Assertion
            .BecauseOf(because, becauseArgs)
            .Given(() => _subject.AllDirectories.ToList())
            .ForCondition(files => files.Contains(path))
            .FailWith(
                "Expected {context:filesystem} to contain directory {0}{reason}, but no directory exists.",
                path);
