...
public class BadRequestException : Exception
{
    ...
    /// <param name="inner">The inner.</param>
    /// <returns></returns>
    public static BadRequestException Create<TEntity>(Expression<Func<TEntity, object>> property, string reason,
        Exception? inner = null) =>
        new(inner, new ValidationFailure(property.GetMemberName(), reason));
    ...
    public static BadRequestException Create(string reason, Exception? inner = null) =>
        new(inner, new ValidationFailure(string.Empty, reason));
    ...
    public static BadRequestException Invalid(string name, Exception? inner = null) =>
        Create($"Invalid {name.Humanize()}", inner);
    ...
    public static BadRequestException Invalid<TEntity>(Exception? inner = null) =>
        Invalid(GetTypeName<TEntity>(), inner);
    ...
    /// <param name="inner">The inner.</param>
    /// <returns></returns>
    public static BadRequestException Invalid<TEntity>(Expression<Func<TEntity, object>> property, string reason,
        Exception? inner = null) =>
        new(inner, new ValidationFailure(property.GetMemberName(), reason));
    ...
    {
        var tokens = validationFailures.Select(f =>
            string.IsNullOrEmpty(f.PropertyName) ? f.ErrorMessage : $"{f.PropertyName}: {f.ErrorMessage}");
